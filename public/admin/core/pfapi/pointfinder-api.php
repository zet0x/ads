...
if ( ! class_exists( 'PointFinder_APIM_API' ) && class_exists( 'PointFinder_APIM' ) ) :

	/**
	 * Creates the Envato API connection.
	 *
	 * @class PointFinder_APIM_API
	 * @version 1.0.0
	 * @since 1.0.0
	 */
	class PointFinder_APIM_API {
		...
		/**
		 * Main PointFinder_APIM_API Instance
		 *
		 * Ensures only one instance of this class exists in memory at any one time.
		 *
		 * @see PointFinder_APIM_API()
		 * @uses PointFinder_APIM_API::init_globals() Setup class globals.
		 * @uses PointFinder_APIM_API::init_actions() Setup hooks and actions.
		 *
		 * @since 1.0.0
		 * @static
		 * @return object The one true PointFinder_APIM_API.
		 * @codeCoverageIgnore
		 */
		 ...
		 *
		 * @see PointFinder_APIM_API::instance()
		 *
		 * @since 1.0.0
		 * @access private
		 * @codeCoverageIgnore
		...
		private function init_globals() {
			// Envato API token.
			$this->token = pointfinder_apim()->get_option( 'token' );
		}
		...
		public function request( $url, $args = array() ) {
			$defaults = array(
				'headers' => array(
					...
					'User-Agent' => 'WordPress - PointFinder ' . pointfinder_apim()->get_version(),
				),
				'timeout' => 20,
			);
			...
			if ( empty( $token ) ) {
				return new WP_Error( 'api_token_error', __( 'An API token is required.', 'pointfindert2d' ) );
			}
			...
			if ( 200 !== $response_code && ! empty( $response_message ) ) {
				...
				return new WP_Error( $response_code, __( 'An unknown API error occurred.', 'pointfindert2d' ) );
			} else {
				$return = json_decode( wp_remote_retrieve_body( $response ), true );
				if ( null === $return ) {
					return new WP_Error( 'api_error', __( 'An unknown API error occurred.', 'pointfindert2d' ) );
				}
				return $return;
			}
		}
		...
		public function deferred_download( $id ) {
			...
			return add_query_arg( $args, esc_url( pointfinder_apim()->get_page_url() ) );
		}
