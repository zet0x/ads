...
if ( ! class_exists( 'PointFinder_APIM_Items' ) ) :

	/**
	 * Creates the theme & plugin arrays & injects API results.
	 *
	 * @class PointFinder_APIM_Items
	 * @version 1.0.0
	 * @since 1.0.0
	 */
	class PointFinder_APIM_Items {
		...
		/**
		 * The PointFinder_APIM_Items Instance
		 *
		 * Ensures only one instance of this class exists in memory at any one time.
		 *
		 * @see PointFinder_APIM_Items()
		 * @uses PointFinder_APIM_Items::init_actions() Setup hooks and actions.
		 *
		 * @since 1.0.0
		 * @static
		 * @return object The one true PointFinder_APIM_Items.
		 * @codeCoverageIgnore
		 */
		 ...
		 *
		 * @see PointFinder_APIM_Items::instance()
		 *
		 * @since 1.0.0
		 * @access private
		 * @codeCoverageIgnore
				...
				foreach ( $installed as $slug => $premium ) {
					...
					if ( $theme->exists() && version_compare( $theme->get( 'Version' ), $premium['version'], '<' ) ) {
						$transient->response[ $slug ] = array(
							...
							'url'         => $premium['url'],
							'package'     => pointfinder_apim()->api()->deferred_download( $premium['id'] ),
						);
					}
				}
			}
			...
			if ( 'plugin_information' === $action && isset( $args->slug ) ) {
				...
				foreach ( $installed as $slug => $plugin ) {
					if ( dirname( $slug ) === $args->slug ) {
						...
						$response->download_link = pointfinder_apim()->api()->deferred_download( $plugin['id'] );
						break;
					}
				}
			}
		...
		public function set_themes( $forced = false, $use_cache = false ) {
			self::$themes = get_site_transient( pointfinder_apim()->get_option_name() . '_themes' );

			if ( false === self::$themes || true === $forced ) {
				$themes = pointfinder_apim()->api()->themes();
				foreach ( pointfinder_apim()->get_option( 'items', array() ) as $item ) {
					...
					if ( 'theme' === $item['type'] ) {
						...
						$request = pointfinder_apim()->api()->item( $item['id'], $request_args );
						if ( false !== $request ) {
							$themes[] = $request;
						}
					}
		...
		public function set_plugins( $forced = false, $use_cache = false, $args = array() ) {
			self::$plugins = get_site_transient( pointfinder_apim()->get_option_name() . '_plugins' );

			if ( false === self::$plugins || true === $forced ) {
				$plugins = pointfinder_apim()->api()->plugins();
				foreach ( pointfinder_apim()->get_option( 'items', array() ) as $item ) {
					if ( empty( $item ) ) {
						continue;
					}
		...
		private function process_themes( $purchased ) {
			...
			set_site_transient( pointfinder_apim()->get_option_name() . '_themes', self::$themes, HOUR_IN_SECONDS );
		}
		...
		private function process_plugins( $purchased, $args = array() ) {
			...
			set_site_transient( pointfinder_apim()->get_option_name() . '_plugins', self::$plugins, HOUR_IN_SECONDS );
		}
	}
